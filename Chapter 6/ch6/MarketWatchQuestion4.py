...
tickers = ['^DJI','^GSPC','MSFT','GS','DAL']
names = ['DOW JONES','S&P500', 'Microsoft', 'Goldman Sachs', 'Delta Airlines']
...
def StockWatch():
    ...
    m2=f'S&P500: {p2}'      
    #obtain live price information for Microsoft stock from Yahoo        
    p3=round(float(si.get_live_price("MSFT")),2)
    m3=f'Microsoft: ${p3}'
    #obtain live price information for Goldman Sachs stock from Yahoo                
    p4=round(float(si.get_live_price("GS")),2)
    m4=f'Goldman Sachs: ${p4}'
    #obtain live price information for Delta Airlines stock from Yahoo                
    p5=round(float(si.get_live_price("DAL")),2)
    m5=f'Delta Airlines: ${p5}'
