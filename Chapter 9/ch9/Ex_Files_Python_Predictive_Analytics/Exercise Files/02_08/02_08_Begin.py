...
para_svr = dict(kernel=["linear","poly"], C=[200,300], epsilon=[0.001,0.0001])

svr=GridSearchCV(SVR(), para_svr, cv=5, verbose=3)
...
newsvr = SVR(kernel="linear", C=300, epsilon=0.001)

#train the model again
newsvr.fit(X_train, y_train)

#predict out of sample
y_test_pred = newsvr.predict(X_test)

#predict in sample
y_train_pred = newsvr.predict(X_train)
...
para_rf = dict(n_estimators=[200,300], max_depth=[5,7], min_samples_leaf=[3,6])

rf=GridSearchCV(RandomForestRegressor(), para_rf, cv=5, verbose=3)
...
#best params
newrf = RandomForestRegressor(n_estimators=200, max_depth=5, min_samples_leaf=6, random_state=0)

newrf.fit(X_train, y_train)

#predict out of sample
y_test_pred_newrf = newrf.predict(X_test)

#predict in sample
y_train_pred_newrf = newrf.predict(X_train)

print(r2_score(y_test, y_test_pred_newrf))
print(r2_score(y_train, y_train_pred_newrf))
